...
if 'gtrends' not in str(cursor.fetchall()):
    ...
    d.to_sql('tmp', conn, if_exists='replace', index=True) # write all fresh spx data (and their dates) to tmp table
...
# pytrends connection
pytrends = TrendReq(hl='en-US', tz=360, timeout=(33, 66))
...
for t in terms:
    ...
    try:
        ...
        if int(get_dates.str.slice(0, 4).min())>=settings['first_year']:
            ...
            # latest relevant daily interval end
            ...
            tmp_get = dailydata.get_daily_data(t,
                                               ...
                                               int(get_dates.str.slice(0, 4).max()), int(get_dates[get_dates.str.slice(0, 4)==get_dates.str.slice(0, 4).max()].str.slice(5, 7).max()),
                                               geo='', wait_time=settings['wait'])
        ...
        time.sleep(np.random.normal(25, 5, 1))
...
# end
time.sleep(10)
print("done getting trends data")
